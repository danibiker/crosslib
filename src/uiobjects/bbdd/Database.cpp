...
*
*/
void Database::setString(unsigned int pos, string var){
...
*
*/
void Database::setInt(unsigned int pos, int var){
    if (pos < statementValues.size()){
...
void Database::setRaw(unsigned int pos, string var){
    if (pos < statementValues.size()){
        statementValues.at(pos) =  var;
    } else {
        throw(Excepcion(ERANGOSQL));
...
//    Traza::print("Database::generarQuery::Buscando sentencia de: " + *query, W_DEBUG);
    ...
    if (pos >= 0){
        ...
        unsigned int numParmsIN = Constant::strToTipo<int>(this->statementDataList.getSubElem(pos).getValue());
        ...
        bool withParms = query->find('?') != string::npos;
        Traza::print("Query: "  + *query, W_PARANOIC);

        //Comprobamos que los tamanyos son correctos
        if (numParmsIN == statementValues.size()){
            for (int i = statementValues.size() - 1; i >= 0 && withParms; i--){
                last = query->find_last_of('?', last);
                *query = Constant::replace(*query, "?", statementValues.at(i), last);
            }
            Traza::print("****** GENERARQUERY ********: " + *query, W_PARANOIC);
            return true;
        } else {
...
*
*/
string Database::getColQuery(vector<vector<string> > *result, unsigned int col){
